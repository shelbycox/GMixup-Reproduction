...
if __name__ == '__main__':
    ...
    parser.add_argument('--res', type=int, default=0) ## resolution modifier we added
    ...
    output_log = csv.writer(open(f'./{dataset_name}/out_{dataset_name}_{str(gmixup)}_{seed}.csv', 'w+'))
    output_log.writerow(['epoch', 'train_loss', 'val_loss', 'test_loss', 'val_acc', 'test_acc'])
    ...
    resolution = int(avg_num_nodes) + res_mod
    M = median_num_nodes
    ...
    f = open(str(seed) +"_"+ str(gmixup) + str(args.model) + dataset_name + '_' + str(int(1000*lam_range[0])) + '_' + str(int(1000*lam_range[1])) + '_' + str(res_mod)  + ".txt", "w")
    f.write("epoch,train_loss,val_loss,test_loss,val_acc,test_acc\n")
    for epoch in range(1, num_epochs):
        ...
        f.write(str(epoch) + ',' + str(train_loss) + ',' + str(val_loss) + ',' + str(test_loss) +',' + str(val_acc)+','+ str(test_acc) + '\n')
    f.close()